...
void process_payload(uint8_t *data, size_t data_len, int crc_correct, block_buffer_t *block_buffer_list, int adapter_no)
{
    ...
    fprintf(stderr, "\r Signal Detected -=-=-=-=-= ");
    fflush(stderr);
    usleep(3000); 	
    ...
    if((block_num > max_block_num || tx_restart) && crc_correct) {     
        ...
        if(last_block_num != -1) {
		    ...
		    fprintf(stderr, "Processing Payload |||||||||||||| = Received Packets = %d ----TX Restart Count = %d -----", rx_status->received_block_cnt++, rx_status->tx_restart_cnt++);	
		    fflush(stderr);
                    usleep(5000);   	
