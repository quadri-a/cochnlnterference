...
void pb_transmit_packet(pcap_t *ppcap, int seq_nr, uint8_t *packet_transmit_buffer, int packet_header_len, const uint8_t *packet_data, int packet_length) {
    ...
    if(packet_length > 0){
        //pck_cnt = pck_cnt++;
	fprintf(stderr, "\r..Packet Length..%d ---", packet_length);
        //fflush(stderr);
        //usleep(5000); 
	//fprintf(stderr, "..Packet Header Length.. \"%d\"", packet_header_len);
	//fprintf(stderr, "..WiFi header Length..\"%ld\"\n", sizeof(wifi_packet_header_t));
    }
...
void pb_transmit_block(packet_buffer_t *pbl, pcap_t *ppcap, int *seq_nr, int port, int packet_length, uint8_t *packet_transmit_buffer, int packet_header_len, int data_packets_per_block, int fec_packets_per_block, int transmission_count) {
	...
	for(i=0; i<data_packets_per_block; ++i) {
		...
		//fprintf(stderr, "..Data blocks.. \"%hhn\"", pbl[i].data);
	}
	...
	//COMMENTED OUT FOR CONTINUOUS TRANSMISSION +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	int x;
	...
	//while(!stop) {
		...
		fprintf(stderr, "\r Getting out of one set of transmission ");
                //fflush(stderr);
                //usleep(3000); 
	}
...
main(int argc, char *argv[])
{
	...
	fifo_t fifo[MAX_FIFOS];
		...
		int param_transmission_count = 0;
    int param_data_packets_per_block = 10;
	...
	//fprintf(stderr, "Param data packet per block ..\"%d\"",param_data_packets_per_block);
 ...
 while (!fBrokenSocket) {
		...
		for(i=0; i<param_fifo_count && ret; ++i) {
			...
			//fprintf(stderr, "FIFO curr pb is %d. \n", fifo[i].curr_pb);		
			//fprintf(stderr, "Data packs per block %d. \n", param_data_packets_per_block);			
